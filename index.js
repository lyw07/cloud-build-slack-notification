...
exports.deployBuild = (req, res) => {
  if (req.headers['user-agent'] != 'Buildkite-Request') {
    return res.send("No builds")
  }

  if (req.headers['x-buildkite-token'] != config.buildkiteToken) {
    console.log("Invalid buildkite token");
    return res.status(401).send('Invalid buildkite token');
  }

  if (req.headers['x-buildkite-event'] == 'build.finished') {
    if (req.body.build.state == 'passed') {
      console.log("Build passed")
      return res.send("Build passed")
    }
    else{
